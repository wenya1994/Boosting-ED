...
class EncoderDecoder_LOCAL8x8(BaseSegmentor):
    ...
    def __init__(self,
        ...
        self.global_model = build_segmentor(global_cfg.model, train_cfg=global_cfg.train_cfg,
                                            test_cfg=global_cfg.test_cfg)
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        """
        batch_size, _, h_img, w_img = img.size()
        # img_crop = img[:, :, 0:h_img - 1, 0:w_img - 1]  # for bsds
        img_crop = img[:, :, 0:h_img-1, 0:w_img]  # for nyud
        # img_crop = img[:, :, 0:h_img-7, 0:w_img-4]  # for pascal
        ...
        # fuse_outs[:, :, 0:h_img - 1, 0:w_img - 1] = fuse_outs_crop  # for BSDS
        fuse_outs[:, :, 0:h_img - 1, 0:w_img] = fuse_outs_crop  # for NYUD
        # fuse_outs[:, :, 0:h_img - 7, 0:w_img - 4] = fuse_outs_crop  # for Pascal
